...
window.onload = function() {
    ...
    comands.addEventListener("keyup", function(event) {
        if (event.keyCode === 13) {
            ...
            if(comands.value.length<3 || (comands.value.charAt(0)!='F' && comands.value.charAt(0)!='B' && comands.value.charAt(0)!='R' && comands.value.charAt(0)!='L' && comands.value.charAt(0)!='U' && comands.value.charAt(0)!='D') || comands.value.charAt(1)!=' ' || isNaN(Number(comands.value.substring(2)))) {
                ...
                if(s.charAt(0)=='F' || s.charAt(0)=='B') {
                    ...
                    let nextX = zx + Math.cos(Math.PI*(zbeta/180))*Math.cos(Math.PI*(zalfa/180))*l;
                    let nextY = zy + Math.cos(Math.PI*(zbeta/180))*Math.sin(Math.PI*(zalfa/180))*l;
                    let nextZ = zz + Math.sin(Math.PI*(zbeta/180))*l;
                    world = world.concat([[[zx,zy,zz],[nextX,nextY,nextZ]]])
                    zx = nextX;
                    zy = nextY;
                    zz = nextZ;
